$(document).ready(function () {
	...
	$(document).on("click","#add_image", function(){		
		var product_id = $('#product_img_id').val();
				...
				$.ajax({
					...
					success: function (data){
						...
						console.log(data);
						 ...
						 $('.row_image').html('<div class="col-md-3"><div class="brand-img-dsp"><div class="brand-img-dsp-inner"><div class="brand-img-dsp-inner1"><div class="brand-img-dsp-inner2"><a href="javascript:void(0);" class="image_delete" id=="'+data.id+'"><img class="img-responsive" src="'+baseurl+data.product_image+'" /></div></div></div><i class="fa fa-times" aria-hidden="true"></i></a></div></div>');
    ...
    $(document).on("click",".next-step", function(){		
		...
		if($(this).attr("id") == "save_general"){						
			if($("#product_name").val() === ""){
			...
			}else if($("#product_sku").val() === null){
				$("#category").addClass("has-error");
				alert('Product could not be blank!');
				return false;
			}else if($("#category").val() === null){
				$("#category").addClass("has-error");
				alert('Category could not be blank!');
				return false;
			}else if($("#page_description").val() === null){
				$("#page_description").addClass("has-error");
				alert('Description could not be blank!');
				return false;
			}
			else{						
				...
				var product_description 	= tinyMCE.get('page_description').getContent();
				$.ajax({
					type:"POST",
			...
			$.ajax({
             ...
             $.ajax({
				...
				data:$('#product_attribute').serialize(),				
	           	...
	           	success:function(result){               		
	           	 console.log(result) ;
                    $('#finishing_product_id').val(result);			 
	           		var $active = $('.wizard .nav-tabs li.active');
					$active.next().removeClass('disabled');					
					nextTab($active);		
					//$active.addClass('disabled');	
