...
defined('BASEPATH') OR exit('No direct script access allowed');
  ...
  class Cart extends MY_Controller {
    ...
    public function index(){
		...
		//$result_banner = $this->home_model->Show_Banner();
		//$data['banner_result'] = $result_banner;
		...
		//if(empty($cart_products) && $this->ion_auth->logged_in()){		
		if($this->ion_auth->logged_in()){
		...
		/*if(!empty($cart_products)){
			foreach($cart_products as $product_img)
			{
				$cart_product_images = $this->home_model->getProductImageV2($product_img['product_id']);
			}
			$data['cart_product_images'] 	= $cart_product_images;
		} */

		//echo"<pre>";print_r($data['cart_product_images']); exit();
