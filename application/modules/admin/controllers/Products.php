...
class Products extends MY_Controller {
	...
	public function edit_step2()
	{
		...
		print_r($data['printing_attributes']);
 ...
 */
	...
	public function edit_step3()
	{
		...
		$data['front_cover_price_data'] = $this->product_model->get_front_cover_price_data($product_id);
		...
		if ($this->input->post()) {
			...
			foreach($this->input->post('coil_binding_cost')['sheets'] as $Key => $value){
				...
				$insert_data_new['range_to'] = $this->input->post('coil_binding_cost')['range_to'][$Key] == 0 ? 9223372036854775807 : $this->input->post('coil_binding_cost')['range_to'][$Key];				$insert_data_new['price'] = $this->input->post('coil_binding_cost')['price'][$Key];
...
/* 			foreach($this->input->post('front_cover_option')['dimension'] as $Key => $value){
				...
				$insert_data_new['product_id'] = $product_id;				$insert_data_new['price'] = $this->input->post('front_cover_option')['price'][$Key];

				
				$insert_data_new['attr_type'] = $this->input->post('front_cover_option')['attr_type'][$Key];

				

				$insert_data_new['dimension'] = $this->input->post('front_cover_option')['dimension'][$Key];

				$insert_data_new['sides'] = $this->input->post('front_cover_option')['sides'][$Key];

				$insert_data_new['ink_type'] = $this->input->post('front_cover_option')['color_type'][$Key];

				$insert_data_new['page_type'] = $this->input->post('front_cover_option')['paper_type'][$Key];
				...
				$this->product_model->insert_front_cover_option_data('tbl_back_cover',$insert_data_new);
			...
			for ($i=0; $i < count($this->input->post('front_cover_type')); $i++) { 
				...
				$front_cover_data[$i]['attr_type'] = "front cover options";
	...
	*/
	...
	{    
		...
		if ($this->input->post()) {
			$product_tab = @$_POST['product_tab'];//$this->input->post('product_tab');
