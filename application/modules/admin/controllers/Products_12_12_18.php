...
class Products extends MY_Controller {
...

	{
		...
		$data['attributelist'] = $this->attribute_model->Show_Attributes();
		...
		if(null !==($this->session->userdata('product_id'))){

		$data['result'] = $this->product_model->Get_Product($this->session->userdata('product_id') , get_current_language()) ;        

		$data['product_category'] = $this->product_model->Get_Product_Category($this->session->userdata('product_id'));

		

		//$data['attributes'] =  $this->product_model->Get_attribute($this->session->userdata('product_id')) ; 

		}

		if(null !==($this->session->userdata('image_id'))){

		$data['product_image'] = $this->product_model->Get_ProductImage($this->session->userdata('image_id'));

		}

		if(null !== $this->session->userdata('attribute_data')){

        

       // $data['attributelist'] = $this->product_model->Get_Product_Attribute_Details($this->session->userdata('product_id'));
...
safe_str_replace(
	 ...
	 // $brandlist = $this->brand_model->Show_Brands(get_current_language());

	  //$attributelist = $this->attribute_model->Show_Attributes();



	  $result = $this->product_model->Get_Product($id,get_current_language());

	  $product_category = $this->product_model->Get_Product_Category($id);

	  $product_image = $this->product_model->Get_ProductImage($id);

	  $data['attributelist'] = $this->attribute_model->Show_Attributes();	

	  //$data['step'] = $this->session->userdata('step');

	  //$this->session->unset_userdata('step','step3');
	  ...
	  // $data['attributelist'] = $this->product_model->Get_Product_Attribute_Details($id);

	  

       $data['printing_attributes'] =  $this->product_model->Get_attribute($id ,$type = 'printing') ;

	   

	    $data['finishing_attributes'] =  $this->product_model->Get_attribute($id ,$type = 'finishing') ;

	   

	   $data['network'] =  $this->product_model->Get_price_range($id) ;
...
safe_str_replace(
	...
	public function add_attributeValue(){
	  ...
	  */
	   ...
	   if(null !==$this->input->post('product_id') && $this->input->post('product_id')!="" ){
		   ...
		   //$this->db->delete('tbl_product_variants', array('product_id' => $this->input->post('product_id') ));

		  // $this->db->delete('tbl_product_range_price', array('product_id' => $this->input->post('product_id') ));
	...
	*/
	      ...
	      foreach($attr_values['cnt'] as $key=>$counts){
			...
			//echo count($attr_values['quantity_from'][$key][$i]) ; 

			

				 for( $j = 0 ;$j < sizeof($attr_values['quantity_from'][$key][$i]);$j++ ){
					...
					'range_from'          =>  $this->input->post('quantity_from')[$key][$i][$j],

					'range_to'            =>  $this->input->post('quantity_to')[$key][$i][$j],

					'price'		          =>  $this->input->post('price')[$key][$i][$j]                   				
        ...
        //foreach($attribute as $key=>$key_val){
			...
			$attribute_data = array(
				...
				'attribute_id'  =>  $this->input->post('attribute')[$key][$j] ,

				'attr_value_id' =>  $this->input->post('attribute_value')[$key][$j],

				'attr_type'     =>  $this->input->post('type'),

				'status'		=>	1,

				'p_varient_group_id' => $product_id.'_'.$j
			 ...
			 $group_id = $product_id.'_'.$this->input->post('cnt')[$key]  ;   

			// echo sizeof ($this->input->post('quantity_to')[$key]) ; exit();

			for( $i = 0 ;$i < sizeof($this->input->post('quantity_from')[$key]);$i++ ){
					...
					'c_varient_group_id'  =>  $group_id ,

					'range_from'          =>  $this->input->post('quantity_from')[$key][$j][$i],

                    'range_to'            =>  $this->input->post('quantity_to')[$key][$j][$i],

					'price'		          =>  $this->input->post('price')[$key][$j][$i]                   				
...
safe_str_replace(
		...
		$this->product_model->Update_Products($product_id, $update_data);
			...
			foreach($storage as $key=>$key_val){

			 

				for( $i = 0 ;$i < count($this->input->post('network')[$key]);$i++ ){
					...
					'storagsafe_str_replace(his->input->post('storage')[$key] ,

					//'storage'    =>  $key_val ,

					'network'    =>  $this->input->post('network')[$key][$i],

                    'price'      =>  $this->input->post('price')[$key][$i],
				   ...
				   // print_r($network_data);
					...
					if(!empty($network_data))			   
						...
						$this->product_model->Insert_Product_varient($network_data); 
