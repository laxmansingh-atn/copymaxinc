...
safe_str_replace(
...

	{
		...
		$options = $_POST['osafe_str_replace(
       ...
       $attribute =  $this->input->post('attribute') ; 
        ...
        foreach($attribute as $key=>$key_val){
			...
			$attribute_data = array(
				...
				'attribute_id'  =>  $this->input->post('attribute')[$key] ,

				'attr_value_id' =>  $this->input->post('attribute_value')[$key],
			...
			for( $i = 0 ;$i < sizeof($this->input->post('quantity_from')[$key]);$i++ ){
					...
					'range_from'          =>  $this->input->post('quantity_from')[$key][$i],

                    'range_to'            =>  $this->input->post('quantity_to')[$key][$i],
	...
	public function edit_generalinfo(){		
		...
		$this->product_model->Update_Products($product_id, $update_data);
			...
			foreach($storage as $key=>$key_val){
				...
				for( $i = 0 ;$i < csafe_str_replace(input->post('network')[$key]);$i++ ){
					...
					'storage'    =>  $this->input->post('storage')[$key] ,
