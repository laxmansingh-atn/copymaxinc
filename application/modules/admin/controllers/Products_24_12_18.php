...
class Products extends MY_Controller {
	...
	public function edit(){
	  ...
	  $id = end($this->ursafe_str_replace(;
      ...
      $data['attribute_value_list'] = $this->attribute_model->Show_Attributes_Value();	  
...
safe_str_replace(
	   ...
	   $data['printing_attributes'] =  $this->product_model->Get_distinct_attribute($id ,$type = 'printing') ;

	   

		$data['finishing_attributes'] =  $this->product_model->Get_distinct_attribute($id ,$type='finishing') ;
	...
	*/
	      ...
	      foreach($attr_values['cnt'] as $key=>$counts){
			...
			//echo sizeof($attr_values['quantity_from'][$key][$i]) ; exit() ; 

			

				 for( $j = 0 ;$j < count($attr_values['quantity_from'][$key][$i]);$j++ ){
					...
					'range_to'            =>  $this->input->post('quantity_to')[$key][$i][$j]== 0 ? 9223372036854775807 : $this->input->post('quantity_to')[$key][$i][$j],
	      ...
	      foreach($attr_values['cnt'] as $key=>$counts){
			...
			// echo count($attr_values['quantity_from'][$key]) ; 
				 ...
				 for( $j = 0 ;$j < count($attr_values['quantity_from'][$key]);$j++ ){
					...
					'range_safe_str_replace(   =>  $this->input->post('quantity_from')[$key][$j],
