    				...
    				<div class="active tab-pane" role="tabpanel" id="step1">
						...
						<form method="post" action="">
                			<div class="row row_block">
                                  	...
                                  	<div class="form-group">
                                    	...
                                    	<?php if(!empty($printing_attributes)): ?>
                                    		<?php foreach ($printing_attributes as $key => $value) : ?>
                                    			<?php if($value['attr_name'] == "dimensions"): ?>
                                    				<input type="checkbox" name="dimensions[]" <?php if(array_search($value['attr_value'], array_column($product_printing_attribute, 'dimension')) !== false) { echo 'checked'; } ?> value="<?=$value['attr_value']?>"><?=$value['attr_value']."<br>"?>
                                    			<?php endif; ?>
                                    		<?php endforeach; ?>
                                    	<?php endif; ?>
                                  	</div>
                    			...
                    			<div class="col-md-6">
                      				<div class="form-group">
                        				<label>Select Paper Type Group </label>
                        				<br>
                                    	<?php if(!empty($printing_attributes)): ?>
                                    		<?php foreach ($printing_attributes as $key => $value) : ?>
                                    			<?php if($value['attr_name'] == "paper type" && $value['page_group_id'] == 0): ?>
                                    				<input type="checkbox" name="page_type[]" <?php if(array_search($value['id'], array_column($product_printing_attribute, 'paper_type_group_id')) !== false) { echo 'checked'; } ?> value="<?=$value['id']?>"><?=$value['attr_value']."<br>"?>
                                    			<?php endif; ?>
                                    		<?php endforeach; ?>
                                    	<?php endif; ?>
                    				</div>
