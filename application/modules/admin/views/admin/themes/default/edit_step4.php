                        ...
                        <div class="custom-field">
                            <div class="add-on side">
                                ...
                                <table class="form-table" id="customFields3" border="0" cellspacing="2" cellpadding="2" width="20%">
                                    <tr>
                                        <?php if(isset($finishing_attributes['divider']['divider'][0]['price'])) : ?>
                                            <td width="20%"><input type="number" class="form-control"  name="divider_price" value="<?=$finishing_attributes['divider']['divider'][0]['price']?>" placeholder="Price" step="any" required /></td>
                                        <?php else : ?>
                            ...
                            <div class="add-on side">
                                <h5>Stapling</h5>
                                <?php if(isset($finishing_attributes['stapling']['stapling'])) : ?>
                                    <?php foreach ($finishing_attributes['stapling']['stapling'] as $key => $value) : ?>
                                        <table class="form-table" id="customFields3" border="0" cellspacing="2" cellpadding="2" width="100%">
                                            <tr>
                                                <td width="20%">
                                                    <select name="stapling_type[]" required>
                                                        <option <?=($value['stapling_type']=="top-left")?'selected':''?> value="top-left">Portrait top left</option>
                                                        <option <?=($value['stapling_type']=="2-staple")?'selected':''?> value="2-staple">2 staples on the spine</option>
                                                    </select>
                                                </td>
                                                <td width="12%">
                                                    <select name="stapling_side[]" required>
                                                        <option <?=(safe_trim($value['page_side'])=="1-sided")?'selected':''?> value="1-sided">1 Sided</option>
                                                        <option <?=(safe_trim($value['page_side'])=="2-sided")?'selected':''?> value="2-sided">2 Sided</option>
                                                    </select>
                                                </td>
                                                <td width="11%">
                                                    <select name="stapling_dimension[]" required>
                                                        <option <?=($value['dimension']=="8.5x5.5")?'selected':''?> value="8.5x5.5">8.5x5.5 </option>
                                                        <option <?=($value['dimension']=="4.25x11")?'selected':''?> value="4.25x11">4.25x11</option>
                                                        <option <?=($value['dimension']=="4.25x5.5")?'selected':''?> value="4.25x5.5">4.25x5.5</option>
                                                        <option <?=($value['dimension']=="11x17")?'selected':''?> value="11x17">11x17</option>
                                                        <option <?=($value['dimension']=="8.5x11")?'selected':''?> value="8.5x11"> 8.5x11</option>
                                                        <option <?=($value['dimension']=="8.5x14")?'selected':''?> value="8.5x14">8.5x14</option>
                                                        <option <?=($value['dimension']=="8.5x7")?'selected':''?> value="8.5x7">8.5x7</option>
                                                    </select>
                                                </td>
                                                <td width="12%">
                                                    <select name="stapling_page_type[]" required>
                                                        <option <?=($value['folding_paper_type']=="collated")?'selected':''?> value="collated">Collated</option>
                                                    </select>
                                                </td>
                                                <td width="15%"><input type="number" class="form-control"  name="stapling_from[]" value="<?=$value['range_from']?>" placeholder="From" step="any" required /></td>
                                                <td width="15%"><input type="number" class="form-control"  name="stapling_to[]" value="<?=($value['range_to'] == '9223372036854775807')?'0':$value['range_to']?>" placeholder="To" step="any" required /></td>
                                                <td width="15%"><input type="number" class="form-control"  name="stapling_price[]" value="<?=$value['price']?>" placeholder="Price" step="any" required /></td>
                                                <?php if($key == 0) : ?>
                            ...
                            <div class="add-on side">
                                <h5>3 Hole Punch</h5>
                                <?php if(isset($finishing_attributes['hole']['hole'])) : ?>
                                    <?php foreach ($finishing_attributes['hole']['hole'] as $key => $value) : ?>
                                        <table class="form-table" id="customFields3" border="0" cellspacing="2" cellpadding="2" width="100%">
                                            <tr>
                                                <td width="20%">
                                                ...
                                                <td width="20%"><input type="number" class="form-control"  name="hole_price[]" value="<?=$value['price']?>" placeholder="Price" step="any" required /></td>
                                                <?php if($key == 0) : ?>
                                                    <td><a href="javascript:void(0);" id="" class="add-bttn addCF3">Add</a></td>
                                                <?php else : ?>
                                                    <td><a href="javascript:void(0);" id="" class="add-bttn remCF">Remove</a></td>
                            ...
                            <div class="add-on side">
                                <h5>Folding</h5>
                                <?php if(isset($folding)) : ?>
                                    <table class="form-table" id="customFields3" border="0" cellspacing="2" cellpadding="2" width="80%">
                                        <tr>
                                            <td width="15%">
                                                <select name="folding_page_type" required>
                                                    <option <?=($folding['folding_paper_type']=="uncollated")?'selected':''?> value="uncollated">Uncollated</option>
                                                </select>
                                            </td>
                                            <td width="35%">
                                                <select class="js-example-basic-multiple" name="page_types[]" multiple="multiple" required>
                                                    <?php $paper_types = explode(",", $folding['paper_group_id']); ?>
                                                    <?php foreach ($master_paper_group as $key => $value) : ?>
                                                        <option value="<?=$value['id']?>" <?php if(in_array($value['id'], $paper_types)){ echo "selected";} ?>><?=$value['attr_value']?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </td>
                                            <td width="30%">
                                                <select class="js-example-basic-multiple" name="folding_dimension[]" multiple="multiple" required>
                                                    <option <?php if(in_array("8.5x5.5", $folding_dimension)){ echo "selected";} ?> value="8.5x5.5">8.5x5.5 </option>
                                                    <option <?php if(in_array("4.25x11", $folding_dimension)){ echo "selected";} ?> value="4.25x11">4.25x11</option>
                                                    <option <?php if(in_array("4.25x5.5", $folding_dimension)){ echo "selected";} ?> value="4.25x5.5">4.25x5.5</option>
                                                    <option <?php if(in_array("11x17", $folding_dimension)){ echo "selected";} ?> value="11x17">11x17</option>
                                                    <option <?php if(in_array("8.5x11", $folding_dimension)){ echo "selected";} ?> value="8.5x11"> 8.5x11</option>
                                                    <option <?php if(in_array("8.5x14", $folding_dimension)){ echo "selected";} ?> value="8.5x14">8.5x14</option>
                                                    <option <?php if(in_array("8.5x7", $folding_dimension)){ echo "selected";} ?> value="8.5x7">8.5x7</option>
                                                </select>
                                            </td>
                                            <td width="15%"><input type="number" class="form-control"  name="folding_price" value="<?=$folding['price']?>" placeholder="Price" step="any" required /></td>
                                        </tr>
                                    </table>
                                <?php else : ?>
                                    <table class="form-table" id="customFields3" border="0" cellspacing="2" cellpadding="2" width="80%">
