...
class About extends MY_Controller {
			...
			if($this->form_validation->run() != FALSE)
			{
				...
				{					
					...
					{						
						...
						$subject = "Tradeworld Registration";
						$message = "<html>
									<head>
									...
									<p>&nbsp;</p>
									<p>Your Registration has been done successfully.<br/>Thank you for joining with tradeword.</p>
									<p>&nbsp;</p>
									<p>Best Regards,<br/>Tradeworld Team</p>
									</body>
									</html>";

						$headers = "From: tradeworld@tradeworld.com" . "\r\n";
					...
					//Cart Session Update
					 ...
					 if(!empty($cart_contents)){
					  ...
					  foreach($cart_contents as $row){ 
						   ...
						   $arr['product_stock']   	= '100';
						   $arr['name']      		= $row['name'];
						   $arr['price']      		= $row['price'];
						   $arr['qty']      		= $row['qty'];						   
						   $arr['options']['purchase_type'] = $row['options']['purchase_type'];					   					   
						   ...
						   unset($arr['options']);	
						   $arr['purchase_type']	= $row['options']['purchase_type'];					   					   
	...
	{
		$this->_container = $this->config->item('bootsshop_template_dir_welcome') . "layout/layout_product.php";
        $this->_modules = $this->config->item('modules_locations');
	...
	public function product_details()
	{
		...
		if($slug != ""){
			...
			$products 					= $this->home_model->getProductDetails($slug);
			//echo"<pre>";print_r($products);exit();

			$data['products'] 			= $products;
			$data['related_products'] 	= $this->home_model->getRelatedProducts($products[0]['category_id'], $products[0]['product_id']);
			//echo htmlentities(end($this->uri->segments));exit;
			$data['category_list'] 		= $this->home_model->getAllCategory();
