...
class AuthorizeNetXMLResponse
{
    ...
    protected function _getElementContents($elementName) 
    {
        $start = "<$elementName>";
        $end = "</$elementName>";
        if (safe_strpos($this->response,$start) === false || safe_strpos($this->response,$end) === false) {
            return false;
        } else {
            $start_position = safe_strpos($this->response, $start)+safe_strlen($start);
            $end_position = safe_strpos($this->response, $end);
