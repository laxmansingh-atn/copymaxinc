...

{
...

	{
        ...
        if (!$buffer_valid || safe_strlen($buffer) < $raw_salt_len) {

            $bl = safe_strlen($buffer);
	...
	public function forgotten_password($identity)
	{
		...
		$bytes = function_exists('random_bytes') ? random_bytes(16) : openssl_random_pseudo_bytes(16); // 16 bytes = 32 hex chars
		$key = bin2hex($bytes); 
		...
		$return = $this->db->affected_rows() == 1;
		$this->trigger_events($return ? ['post_forgotten_password', 'post_forgotten_password_successful'] : ['post_forgotten_password', 'post_forgotten_password_unsuccessful']);
		return $return;
	}
	...
	public function forgotten_password_complete($code, $salt=FALSE, $password) 
...

	{
        ...
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            ...
            } else if (safe_strlen($identity) > 0) $this->db->or_where('login', $identity);
