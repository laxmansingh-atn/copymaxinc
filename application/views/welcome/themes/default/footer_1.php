...
.datepicker.dropdown-menu {
  ...
  .datepicker.dropdown-menu tr td.activeClass{
	   #background: #ff0000d6;
	   #background: #83c183a8;
  ...
  .datepicker.dropdown-menu tr td.activeClass:hover
  {
    #background: #ff0000;
    #background: #83c183fa;
    background: #f3b1a7;
  ...
  .datepicker.dropdown-menu tr td.success{
	   background-color: #35971d;
	   color: #ffffff;
  }
  
  .datepicker.dropdown-menu tr td.success:hover
  {
    background: #59c140;
  }
thead {
          ...
          <div class="row clearfix">
            ...
            <div class="col-md-4 col-sm-4 no_pad_right">
              ...
              <div class="contact_info">
                ...
                <p>Phone : <a href="tel:18442679629">1-844-267-9629</a></p>
     ...
     <form method="POST" id="product_info_modal_form" name="product_info_modal_form">
      ...
      <div class="modal-body">
        ...
        <div class="row">
          ...
          <h4>Contact No : 1-844-267-9629 </h4>
...
<div id="callModal" class="modal fade" role="dialog">
	<div class="modal-dialog modal-md" style="margin: 5px auto;">
		<!-- Modal content-->

		<div class="modal-content">
			<div class="modal-body " style="padding: 5px;">
				<section class="body_wrapper">
					<div style="border: 5px solid black;padding: 15px;text-align:center;">
						<h2 style="padding:14px;font-size:30px;"><b>CALL FOR A LAST MINUTE QUOTE</b></h2>
						<div class="row">
							<p>We do offer next days services</p>
						</div>
						<div class="row">
							<span>Call 1-844-267-9629</span>
						</div>
						<div class="row">
							<div class="col-md-3">
...
jQuery(document).ready(function($) {
  ...
  {
	//setTimeout(function() {$("#signup_login_modal").modal('show')}, 2000);
  }
	...
	if(today_weekday == 6){
		completion_avl_days_0 = format_date(new Date(new Date().getTime()+ 2 * (24 * 60 * 60 * 1000)));
	}else if(today_weekday == 0){
		completion_avl_days_0 = format_date(new Date(new Date().getTime() + 1 * (24 * 60 * 60 * 1000)));
	}else{
		completion_avl_days_0 = format_date(new Date(new Date().getTime()));
	}
	//console.log("today_weekday: ",today_weekday);
	//console.log("completion_avl_days_0: ",completion_avl_days_0);
    if(today_weekday == 0){
		...
		completion_avl_days_4=format_date(new Date(new Date().getTime() + 5 * (24 * 60 * 60 * 1000)));
		completion_avl_days_5=format_date(new Date(new Date().getTime() + 6 * (24 * 60 * 60 * 1000)));
    }
    else if(today_weekday == 1){
		
		completion_avl_days_1=format_date(new Date(new Date().getTime() + 1 * (24 * 60 * 60 * 1000)));
        completion_avl_days_2=format_date(new Date(new Date().getTime() + 2 * (24 * 60 * 60 * 1000)));
        completion_avl_days_3=format_date(new Date(new Date().getTime() + 3 * (24 * 60 * 60 * 1000)));
		completion_avl_days_4=format_date(new Date(new Date().getTime() + 4 * (24 * 60 * 60 * 1000)));
		completion_avl_days_5=format_date(new Date(new Date().getTime() + 7 * (24 * 60 * 60 * 1000)));
	}
        ...
        completion_avl_days_2=format_date(new Date(new Date().getTime() + 2 * (24 * 60 * 60 * 1000)));
        completion_avl_days_3=format_date(new Date(new Date().getTime() + 3 * (24 * 60 * 60 * 1000)));
        completion_avl_days_4=format_date(new Date(new Date().getTime() + 6 * (24 * 60 * 60 * 1000)));
		completion_avl_days_5=format_date(new Date(new Date().getTime() + 7 * (24 * 60 * 60 * 1000)));
	  ...
	  //if (hour < 11 && today_weekday != 6 && today_weekday != 0){
	  if (today_weekday != 6 && today_weekday != 0){
		//completion_avl_dates=[completion_avl_days_0,completion_avl_days_1,completion_avl_days_2,completion_avl_days_3];
		completion_avl_dates=[completion_avl_days_1,completion_avl_days_2,completion_avl_days_3,completion_avl_days_4];
	  }else{
		 // alert('in else');		 //completion_avl_dates=[completion_avl_days_1,completion_avl_days_2,completion_avl_days_3,completion_avl_days_4];
		 completion_avl_dates=[completion_avl_days_0,completion_avl_days_1,completion_avl_days_2,completion_avl_days_3];
      }   
    //console.log(completion_avl_dates);
    ...
    $("#cart_datepicker_completion").datepicker({
      ...
      beforeShowDay:
        function(date){
			...
			var allDates = ((date.getMonth() > 8) ? (date.getMonth() + 1) : ('0' + (date.getMonth() + 1))) + '/' + ((date.getDate() > 9) ? date.getDate() : ('0' + date.getDate())) + '/' + date.getFullYear();
			
			if(completion_avl_dates.indexOf(allDates) == 0)
				return {classes : "activeClass"};
			if(completion_avl_dates.indexOf(allDates) == 3)
    ...
    autoclose: true
	  ...
	  $("#price_check_btn").removeAttr('disabled');
	 ...
	 if(completion_avl_dates[1] == $(this).val())
	 {
		total = '<?= number_format($details["total"]*25/100 + $details["total"], 2) ?>';
	 }else if(completion_avl_dates[2] == $(this).val()){
		 total = '<?= number_format($details["total"]*12/100 + $details["total"], 2) ?>';
	 }else{
		  total = '<?= number_format($details["total"], 2) ?>';
	 }
	 $('.total1').html("<strong> Total : $"+total+"</strong>");
	 ...
	 if(minDate_weekday == 0){
      ...
      $('#current_date').val($(this).val());
		...
		if(completion_avl_dates[0] == $(this).val())
		{
			$("#callModal").modal('show');
			$("#cart_datepicker_completion").val('');
			$("#price_check_btn").attr('disabled', true);
			$("#add_to_cart").attr('disabled', true);
			$("#ups_response_div").remove();
			$("#quote_hide_show").remove();
		}
  });
  
  $('#cart_datepicker_completion').on('input', function(){
	if(completion_avl_dates[0] == $(this).val())
	{		
       $("#cart_datepicker_completion").val('');
	}
...
$("#upload").change(function() {
 ...
 $(document).ready(function() {
	...
	/*   if (no_page > 420 && no_of_sides == "1-sided") {
		  
        bootbox.alert("<span style='color:red'>Maximum number of sheets exceeded.Please use Velo Binding</span> <br> Maximum number of pages for 1-sided are 420 <br> Maximum number of pages for 2-sided are 840 <br> <span style='color:red'>If you are doing 2-sided please select Number of sides first and than number of pages</span>").find('.modal-content').css({'font-weight' : 'bold', 'font-size': '18px'});
...
/* 	   if (no_page > 840 && no_of_sides == "2-sided") {
...
$(document).on('keyup', '#additional_charges' , function(){
  ...
  $.ajax({
      ...
      //alert(res);
        ...
        var sales_tax = parseFloat((Number($('#subtotal').val()) + Number($('#shipping_amount_hid').val()) + Number($('#additional_charges').val())) * <?=SALES_TAX_PER?>).toFixed(2);
		//alert(sales_tax);
      ...
      $.ajax({
          ...
          success:function(result){
            ...
            if (result == 0) {
			  ...
			  after_discount = (after_discount > 0) ? after_discount :0;
